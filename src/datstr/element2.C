...
double Element::calc_elem_edge_wetness_factor(int ineigh, double dt) {
  ...
  if((iwetnode==8)||!(state_vars[0]>0/*>GEOFLOW_TINY*/)) return Awet;  //one or zero;
  ...
  *phi_flag=0;
  ...
  for (i=0; i<3; i++)
    for (j=0; j<NUM_STATE_VARS; j++){
      hrfv[i][j]=hfv[i][j];
      if (isnan(hrfv[i][j])) {
        printf("Nan in xdir flux i=%d j=%d phi=%f key=%d key1=%d\n",i,j,state_vars[0],key[0],key[1]); 
        exit(1);}
    }
  return;
}
...
void calc_intercell_vel(double hfvl[3][NUM_STATE_VARS],double hfvr[3][NUM_STATE_VARS] ,int dir,double *inter_cell_vel){
  ...
  double p1= 0.11270166537, p2 = 0.5, p3 = 0.88729833462, w1 = 0.27777777777, w2 =0.44444444444 , w3 = 0.27777777777;
  ...
  double intercell=w1*(vel+p1*(neigh_vel-vel))+w2*(vel+p2*(neigh_vel-vel))+w3*(vel+p3*(neigh_vel-vel));
  inter_cell_vel[0] = .5*(intercell-dabs(intercell));
  inter_cell_vel[1] = .5*(intercell+dabs(intercell));

  if(isnan(inter_cell_vel[0])||isnan(inter_cell_vel[1])) exit(1);
  return;
}
    ...
    double *phi, double *vel,int *phi_flag)
{
  ...
  if(dir%2==0) 
    xdirflux(matprops_ptr,dz,wetnessfactor,hfv,hrfv,vel, phi ,phi_flag);
  else if(dir%2==1) 
...
int Element::if_pile_boundary(HashTable *ElemTable, double contour_height){
  ...
  //  int ineigh;
  //  Element* ElemNeigh;
  //  // if (!(state_vars[0]>=0)){//((state_vars[0]<0)||(isnan(state_vars[0])))  {
  //  //   int i;
  //  //   printf("ElemKey: %u      %u\n", *(this->pass_key()),*(this->pass_key()+1));
  //  //   printf("state_vars: \n");
  //  //   for (i=0; i<NUM_STATE_VARS; i++)
  ...
  //  if(dabs(state_vars[0])<=contour_height)
  //  {
