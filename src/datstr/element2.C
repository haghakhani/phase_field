...
Element::~Element() {
	if (bcptr)
		delete bcptr;
	//printf("Hello I am destroying the element \n");
...
double Element::calc_elem_edge_wetness_factor(int ineigh, double dt) {
	...
	if ((iwetnode == 8) || !(state_vars[0] > 0/*>GEOFLOW_TINY*/))
...
double Element::convect_dryline(double VxVy[2], double dt) {
	...
	if ((state_vars[0] > 0/*<=GEOFLOW_TINY*/) && (dt > 0.0)) {
		Awet = 0.0;
		return Awet;
	}
...
void Element::xdirflux(MatProps* matprops_ptr, double dz, double wetnessfactor,
	...
	if ((state_vars[1] < GEOFLOW_TINY))  //&&(state_vars[1]==0))
	{
		...
		//fluxes
		hfv[1][0] = hfv[0][0]; // we put here to be able compute \frac{\partial \phi}{\partial x} in flux
		hfv[1][1] = hfv[0][1] * Vel;
		hfv[1][2] = hfv[0][2] * Vel + 0.5 * a * a * hfv[0][1];
		hfv[1][3] = hfv[0][3] * Vel;
		hfv[1][4] = 0;
		hfv[1][5] = 0;
...
void Element::ydirflux(MatProps* matprops_ptr, double dz, double wetnessfactor,
	...
	if ((state_vars[1] < GEOFLOW_TINY))  //&&(state_vars[1]==0))
	{
		...
		//fluxes
		hfv[1][0] = hfv[0][0]; //we put here to be able compute \frac{\partial \phi}{\partial u} in flux
		hfv[1][1] = hfv[0][1] * Vel;
		hfv[1][2] = hfv[0][2] * Vel;    // + 0.5*temp2;
		hfv[1][3] = hfv[0][3] * Vel + 0.5 * hfv[0][1] * a * a;
		hfv[1][4] = 0;
		hfv[1][5] = 0;
...
void Element::calc_edge_states(HashTable* El_Table, HashTable* NodeTable, MatProps* matprops_ptr,
	...
	double vel1, phi1, vel2, phi2;
	int phi_flag1, phi_flag2;
		...
		if (neigh_proc[zp] == -1) {
			...
			*outflow += (nm->flux[0]) * dx[!side];
			...
			if (neigh_proc[zp + 4] == myid) {
				...
				riemannflux(hfv, hfv2, nm2->flux, side);
				riemannflux(hrfv, hrfv2, nm2->refinementflux, side);
				...
				riemannflux(hrfv, hrfv2, ghostflux, side);
				for (ivar = 0; ivar < NUM_STATE_VARS; ivar++)
			...
			if (np->info == S_S_CON) {
				...
				if (*(elm1->get_neigh_proc() + (zelmpos + 4) % 8) == myid) {
					...
					riemannflux(hfv2, hfv1, np2->flux, side);
					riemannflux(hrfv2, hrfv1, np2->refinementflux, side);
					...
					riemannflux(hfv2, hfv1, ghostflux, side);
					for (ivar = 0; ivar < NUM_STATE_VARS; ivar++)
						nm1->flux[ivar] = 0.5 * (np->flux[ivar] + ghostflux[ivar]);

					riemannflux(hrfv2, hrfv1, ghostflux, side);
					for (ivar = 0; ivar < NUM_STATE_VARS; ivar++)
		...
		if (neigh_proc[zm] != myid) {
			...
			if (neigh_proc[zm] == -1) {
				...
				*outflow -= (np->flux[0]) * dx[!side];
				...
				riemannflux(hfv1, hfv, nm->flux, side);
				riemannflux(hrfv1, hrfv, nm->refinementflux, side);
				...
				if (neigh_proc[zm + 4] == myid) {
					...
					riemannflux(hfv2, hfv, np2->flux, side);
					riemannflux(hrfv2, hrfv, np2->refinementflux, side);
					...
					riemannflux(hfv2, hfv, ghostflux, side);
					for (ivar = 0; ivar < NUM_STATE_VARS; ivar++)
						nm->flux[ivar] = 0.5 * (nm->flux[ivar] + ghostflux[ivar]);

					riemannflux(hrfv2, hrfv, ghostflux, side);
					for (ivar = 0; ivar < NUM_STATE_VARS; ivar++)
...
void Element::calc_lap_phi(HashTable* El_Table, HashTable* NodeTable) {
	...
	lap_phi[0] = 2 * (dp / dxp - dm / dxm) / (dxp + dxm); //(dp - dm) / (dxp + dxm);
	...
	lap_phi[1] = 2 * (dp / dxp - dm / dxm) / (dxp + dxm);	//(dp - dm) / (dxp + dxm);
...
void Element::calc_flux_balance(HashTable* NodeTable) {
	...
	for (j = 1; j < 4; j++)
		flux[j - 1] = dabs(nd_xp->refinementflux[j] - nd_xn->refinementflux[j])
	...
	if (Influx[1] > 0.0) {
		...
		for (ineigh = 0; ineigh < 8; ineigh++)
			if (neigh_proc[ineigh] >= 0.0) { //don't check outside map boundary or duplicate neighbor
				...
				assert(ElemNeigh);
				assert(*(ElemNeigh->get_influx() + 1) >= 0.0);
				if (*(ElemNeigh->get_influx() + 1) != 0.0)
