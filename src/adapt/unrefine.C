		...
		if (*(buck + i)) {
			...
			while (currentPtr) {
				...
				if (Curr_El->get_adapted_flag() == NOTRECADAPTED
						&& !(Curr_El->if_phase_boundary(El_Table))) {//if this is a refined element don't involve!!!
					...
					if ((Curr_El->get_gen() > MIN_GENERATION)
						...
						Curr_El->find_brothers(El_Table, NodeTable, target,
								myid, matprops_ptr, &NewFatherList,
								&OtherProcUpdate);
...
void delete_oldsons(HashTable* El_Table, HashTable* NodeTable, int myid,
	...
	for (inode = 4; inode < 8; inode++) {
		...
		if (!NdTemp) {
			...
			fflush(stdout);
			NodeBackgroundCheck(El_Table, NodeTable, EmFather->node_key[inode],
					stdout);
		}
		assert(NdTemp);
	...
	for (iupdate = 0; iupdate < NewFatherList->get_num_elem(); iupdate++) {
		...
		for (ineigh = 0; ineigh < 8; ineigh++)
			if (EmFather->neigh_proc[ineigh] == myid) {
				...
				EmNeigh = (Element*) El_Table->lookup(
						EmFather->neighbor[ineigh]);
				...
				if (EmNeigh->adapted != NEWFATHER) {
					...
					for (ineighme = 0; ineighme < 4; ineighme++)
						if (compare_key(EmNeigh->neighbor[ineighme],
								EmFather->son[isonA])
					...
					EmNeigh->neigh_gen[ineighme] = EmNeigh->neigh_gen[ineighme
					...
					if (EmNeigh->adapted == NEWFATHER) {
						...
						EmFather->neigh_gen[ineigh] = EmFather->neigh_gen[ineigh
								+ 4] = EmNeigh->generation;
...
void unrefine_interp_neigh_update(HashTable* El_Table, HashTable* NodeTable,
	...
	int *num_send = CAllocI1(nump), *num_recv = CAllocI1(nump), *isend =
			CAllocI1(nump);
	...
	for (iproc = 0; iproc < nump; iproc++)
		if ((iproc != myid) && (num_recv[iproc] > 0))
			ierr = MPI_Irecv((void * ) recv[iproc],
					4 * KEYLENGTH * num_recv[iproc], MPI_UNSIGNED, iproc,
		...
		for (ineigh = 0; ineigh < 8; ineigh++) {
			...
			if ((neigh_proc >= 0) && (neigh_proc != myid)) {
				...
				for (ikey = 0; ikey < KEYLENGTH; ikey++) {
					...
					send[neigh_proc][(4 * isend[neigh_proc] + 0) * KEYLENGTH
							+ ikey] = EmFather->neighbor[ineigh][ikey];
					//the OLDSON who will introduce his NEWFATHER to his
					//neighbor on another processor
					send[neigh_proc][(4 * isend[neigh_proc] + 1) * KEYLENGTH
							+ ikey] = EmFather->son[ison][ikey];
					//the NEWFATHER element
					send[neigh_proc][(4 * isend[neigh_proc] + 2) * KEYLENGTH
							+ ikey] = EmFather->key[ikey];
					send[neigh_proc][(4 * isend[neigh_proc] + 3) * KEYLENGTH
							+ ikey] = NdTemp->key[ikey];
				}
	...
	for (iproc = 0; iproc < nump; iproc++)
		if ((iproc != myid) && (num_send[iproc] > 0))
			ierr = MPI_Isend((void * ) send[iproc],
					4 * KEYLENGTH * num_send[iproc], MPI_UNSIGNED, iproc,
		...
		do {
			...
			for (iproc = 0; iproc < nump; iproc++)
				if ((iproc != myid) && (num_recv[iproc] > 0)) {
					if (myid == TARGET_PROC)
						printf(
								"myid=%d iproc=%2d unref_interp_neigh_update 5.0\n",
								myid, iproc);
					fflush(stdout);
					...
					if (ifrecvd) {
						if (myid == TARGET_PROC)
							printf(
									"myid=%d iproc=%2d unref_interp_neigh_update 6.0\n",
									myid, iproc);
						fflush(stdout);
						...
						for (iopu = 0; iopu < num_recv[iproc]; iopu++) {
							...
							if (EmTemp->adapted == OLDSON) {
								...
								EmFather = (Element*) El_Table->lookup(
										EmTemp->father);
								assert(EmFather);
								...
								if (NdTemp) {
									...
									NodeTable->remove(NdTemp->key, 0, stdout,
											myid, 12);
									delete NdTemp;
								}
								NdTemp = (Node *) NodeTable->lookup(
								...
								NdTemp = (Node *) NodeTable->lookup(
										EmTemp->node_key[ineighmod4 + 4]);
								assert(NdTemp);
								if (EmTemp->neigh_gen[ineighmod4]
										== EmTemp->generation)
									NdTemp->info = SIDE;
						...
						if (myid == TARGET_PROC)
							printf(
									"myid=%d iproc=%2d unref_interp_neigh_update 7.0\n",
									myid, iproc);
						fflush(stdout);
