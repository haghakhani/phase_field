		...
		StatProps* statprops, DISCHARGE* discharge, double d_time)
{
		...
		if(*(buck+i))
		{
			...
			while(currentPtr){
				...
				if((Curr_El->get_adapted_flag()>0)&&
					...
					{
						if(state_vars[1] > max_height) max_height = state_vars[1];

						double* xy = Curr_El->get_coord();

						if(state_vars[1] >= statprops->hxyminmax)
						{
						...
						if(testpointdist2<testpointmindist2) {
							...
							testpointreach=((state_vars[1]>=testpointheight)?1:0);
						}

						dA = *(Curr_El->get_dx())**(Curr_El->get_dx()+1);
						area+=dA;
						dVol=state_vars[1]*dA;
						testvolume+=dVol;
						xC+=xy[0]*dVol;
						yC+=xy[1]*dVol;
						...
						u_ave+=sqrt(state_vars[5]*state_vars[5]+state_vars[5]*state_vars[5])*dA;
								...
								(!((state_vars[5]<=0.0)||(0.0<=state_vars[5]))))
						{
							...
							printf("prev {Vx_s, Vy_s, Vx_f, Vy_f}={%12.6g,%12.6g,%12.6g,%12.6g}\n",
									...
									*(Curr_El->get_prev_state_vars()+4)/(*(Curr_El->get_prev_state_vars()+1)),
									*(Curr_El->get_prev_state_vars()+5)/(*(Curr_El->get_prev_state_vars()+1)));
							printf("this {Vx_s, Vy_s, Vx_f, Vy_f}={%12.6g,%12.6g,%12.6g,%12.6g}\n",
									state_vars[2]/state_vars[1],
									state_vars[3]/state_vars[1],
									state_vars[4]/state_vars[1],
									state_vars[5]/state_vars[1]);
	...
	if(myid == 0) 
	{
		...
		printf(" end of time step %d the time is %d:%02d:%g (hrs:min:sec),\n"
