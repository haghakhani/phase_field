...
PetscErrorCode MakeRHS(ContData *ctx, Vec b) {
	...
	ierr = VecCreateSeq(PETSC_COMM_SELF, num_elem_proc[rank], &blocal);
	CHKERRQ(ierr);
	ierr = VecGetArray(blocal, &b_ptr);
	CHKERRQ(ierr);
	...
	ierr = VecRestoreArray(blocal, &b_ptr);
	CHKERRQ(ierr);
	//ierr = VecGetSize(b,&xsize);
	//cout<<"size b in RHS 2:  "<<xsize<<endl;

	ierr = VecScatterBegin(*vscat, blocal, b, INSERT_VALUES, SCATTER_FORWARD);
	CHKERRQ(ierr);
