	...
	if (!Initialize_GIS_data(GISDbase, location, mapset, topomap)) {
		...
		if (havelimits) {
			...
			if ((limits[0] > xmin) && (limits[0] < xmax))
				xmin = limits[0];
			if ((limits[1] > ymin) && (limits[1] < ymax))
				ymin = limits[1];
			if ((limits[2] > xmin) && (limits[2] < xmax))
				xmax = limits[2];
			if ((limits[3] > ymin) && (limits[3] < ymax))
				ymax = limits[3];
		}
		...
		if (*material_count == 1) //only 1 material
			...
			for (j = 1; j <= ny; j++)
				for (i = 1; i <= nx; i++) {
					...
					elem_node[ielem][6] = 2 * i + j * (3 * nx + 2);
					elem_node[ielem][7] = i + j * (2 * nx + 1) + (j - 1) * (nx + 1);
					Get_raster_id(matres, xy[ielem][0], xy[ielem][1], &(elem_mat[ielem]));
					elem_loc[ielem][0] = i - 1;
					elem_loc[ielem++][1] = j - 1;
				}
		...
		for (ielem = 0; ielem < *element_count; ielem++) {
			for (j = 0; j < 8; j++) {
				...
				if ((*node)[w].get_nodeid() != elem_node[ielem][j]) {
					w = 0;
					while ((*node)[w].get_nodeid() != elem_node[ielem][j])
						w++;
