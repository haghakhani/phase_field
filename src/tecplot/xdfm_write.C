    ...
    MatProps *matprops_ptr, MapNames *mapnames, const int mode)
{
  ...
  vector<double> phi,pheight,xmom,ymom,xcoord,ycoord,zcoord;
  vector<int> C1,C2,C3,C4;
  int num_nodes=0,num_elm=0;
  int i,j,k;
  double *state_vars,*coord,elevation;
  int conn[4];
  //printf("this the pile_height that I want to write .............\n");
  ...
  for (i=0; i<buckets; i++)
  {
	...
	pheight.push_back(state_vars[1]*matprops_ptr->HEIGHT_SCALE);
  ...
  //Phi
  copy(phi.begin(),phi.end(),vars);
  GH5_write_state_vars(h5fid,num_elm,vars,"PHI");
  ...
  xmlf << "</Geometry>"<<endl;
  //phi
  xmlf << "<Attribute Type=\"Scalar\" Center=\"Cell\" Name=\"Phi\">"<<endl;
  xmlf << "<DataItem DataType=\"Float\" Precision=\"8\" ";
  xmlf << "Dimensions=\""<<num_elm<<" 1\" Format=\"HDF\">"<<endl;
  xmlf << "\t\t"<<hdf5file<<":/Properties/PHI"<<endl;
  xmlf << "</DataItem>"<<endl;
  xmlf << "</Attribute>"<<endl;
  // pile-height
